...
class TestSchellingModel(unittest.TestCase):
    ...
    def test_convergence(self):
        model = SchellingModel(num_groups=3, num_neighbors=2, board_size=10, empty_percentage=0.2, tolerance_threshold=0.3)
    ...
    def test_no_empty_cells(self):
        model = SchellingModel(num_groups=2, num_neighbors=2, board_size=10, empty_percentage=0.05, tolerance_threshold=0.5)
    ...
    def test_high_tolerance(self):
        model = SchellingModel(num_groups=3, num_neighbors=2, board_size=10, empty_percentage=0.1, tolerance_threshold=0.9)
        result = model.init(max_generations=100)
